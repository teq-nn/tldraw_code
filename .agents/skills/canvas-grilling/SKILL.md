...
- **Show UIs as prototypes, do not describe them.** When the decision is between 2 or 3 UI alternatives (a layout, a control, a flow of screens), call `compare` with one `html` item per alternative instead of `spec`: each becomes a clickable prototype side by side, with the question card below. Each prototype is one self-contained HTML document: read the app's CSS and components first and inline its variables, classes and markup, so it looks like the real thing; everything must be inline, since the sandbox loads nothing from the network. Give it a label of a few words and a one-sentence caption on what sets it apart. The user clicks through it on the canvas.
