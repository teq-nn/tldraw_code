...
- **Questions go through `ask`, one per call, one at a time.** Each is one short sentence with 2 to 4 options of a few words and your recommendation (exactly one of the options). The card adds "Keep grilling" itself. Alternatives of structure or UI become short named options too.
...
4. **Ask.** Call `ask` about that node.
   - "No answer yet": call `ask` again with exactly the same arguments, as often as it takes. The user is thinking or on a break; the card stays open.
