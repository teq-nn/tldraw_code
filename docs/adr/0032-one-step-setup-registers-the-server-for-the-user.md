...
- **`pnpm setup:mcp`** runs `scripts/install.sh`, which checks every prerequisite before changing anything (Node >= 22, pnpm >= 10, the `claude` CLI, `curl` for the hook, and the bridge port `CANVAS_BRIDGE_PORT`, default 4477, being free for the smoke check), reports all failures at once with a concrete remedy, then runs `pnpm install --frozen-lockfile` and `pnpm build:board-script` and hands over to `scripts/setup-mcp.ts`. The script can also be run directly as `scripts/install.sh`.
...
- **Idempotent.** A second run changes nothing. An existing user-scope `tldraw-canvas` entry that differs (another checkout, another command) is replaced, and the script prints what it replaced.
...
- **Verification.** `claude mcp get tldraw-canvas`, run outside the repo, must report the server connected, or the script fails with its output. Then `pnpm smoke` runs end to end through the canvas; since that needs tldraw offline running, a failed smoke check only warns and says how to rerun it. `--no-smoke` skips it (and the port check). The script ends with the next steps: start tldraw offline, `/mcp`, the channel flag.
