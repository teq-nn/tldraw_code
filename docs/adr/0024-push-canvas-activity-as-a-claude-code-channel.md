...
- **Pushes are rationed.** Only a new or rewritten `&agent` note wakes Claude; other notes, drawings, moves, edits and deletions do not. Nothing is pushed while a canvas tool call runs (its result carries the digest; a waiting `ask` gets its answer on its own). A tagged note Claude already heard of, by push or by `read_canvas`, does not wake it again; and a tagged note the user did not write in this tab (left over, loaded from storage, synced from another tab) never wakes it, so untagged notes are not blamed for it; and a digest it already got is not pushed twice in a row.
