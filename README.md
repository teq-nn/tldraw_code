...
```sh
pnpm test        # vitest: protocol, frontier, question and diagram schemas and the diff of alternatives, MCP tools (incl. ask, compare, render_diagram, read_canvas and the activity digest) against a fake canvas, canvas bridge client, command handlers, graph layout, diagram frames and comparisons, question card and answer watcher, canvas reads and activity tracking, collapsing answered cards, the tracker sync (ticket-to-node mapping, body conventions, `sync_wayfinder_map` against a fake GitHub), and the canvas-grilling skill against the registered tools
